...
volatile unsigned int __window_counter = 0;
...
#define TIME_AND_TSC(TITLE, CODE)                                     \
    if(!strcmp(TITLE, TOSTRING(BENCHMARK))) {                                                                    \
        unsigned long jj;                                          \
        int ret = 0;                                                        \
                                                                        \
        mbedtls_printf(HEADER_FORMAT, "Result");                             \
        fflush(stdout);                                                   \
                                                                        \
        __window_counter = 0;                                   \
        ...
        {   const uint32_t wc = __window_counter;                               \
            mbedtls_printf("%9lu\n",              \
                           wc           \
                           );                          \
        }                                                                   \
...
int app_main(int argc, char *argv[])
{
    ...
    if (todo.hmac_drbg) {
        ...
        TIME_AND_TSC("HMAC_DRBG_SHA-1_(NOPR)",
        ...
        TIME_AND_TSC("HMAC_DRBG_SHA-1_(PR)",
                     mbedtls_hmac_drbg_random(&hmac_drbg, buf, BUFSIZE));
#endif
        ...
        TIME_AND_TSC("HMAC_DRBG_SHA-256_(NOPR)",
                     mbedtls_hmac_drbg_random(&hmac_drbg, buf, BUFSIZE));
        ...
        mbedtls_hmac_drbg_set_prediction_resistance(&hmac_drbg,
                                                    MBEDTLS_HMAC_DRBG_PR_ON);
        TIME_AND_TSC("HMAC_DRBG_SHA-256_(PR)",
                     mbedtls_hmac_drbg_random(&hmac_drbg, buf, BUFSIZE));
#endif
